...
class AutoTestSub(vehicle_test_suite.TestSuite):
    ...
    def wait_for_distance(self, distance, timeout=100):
        original_location = self.mav.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
        original_location.lat *= 1e-7
        original_location.lon *= 1e-7
        start = self.get_sim_time()
        while self.get_sim_time() - start < timeout:
          current_location = self.mav.recv_match(type='GLOBAL_POSITION_INT', blocking=True)
    ...
    def IgnoreGPSDrift(self):
        ...
        self.arm_vehicle()
        self.change_mode('POSHOLD')
        self.set_parameter("SIM_GPS2_ACC", 2)
        self.watch_position_maintained(delta=0.3, timeout=60.0)
        # dive down to 10m
        self.set_rc(Joystick.Throttle, 1300)
        self.wait_altitude(altitude_min=-10, altitude_max=-9, relative=False, timeout=60)
        self.set_rc(Joystick.Throttle, 1500)
