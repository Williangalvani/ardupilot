...
void AC_PosControl::run_z_controller()
{
    ...
    if (_vibe_comp_enabled) {
        ...
        _thr_out = POSCONTROL_VIBE_COMP_P_GAIN * thr_per_accelz_cmss * _accel_target.z + _pid_accel_z.get_i() * 0.001f;
    } else {
        _thr_out = _pid_accel_z.update_all(_accel_target.z, z_accel_meas, (_motors.limit.throttle_lower || _motors.limit.throttle_upper)) * 0.001f;
    }
    _thr_out += _motors.get_throttle_hover();

    // send throttle to attitude controller with angle boost
    _attitude_control.set_throttle_out(_thr_out, true, POSCONTROL_THROTTLE_CUTOFF_FREQ);
